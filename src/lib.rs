...
impl<
    ...
    FromCap: Copy + serde::Serialize,
    ToCap: Copy + serde::Serialize,
> DebugRepr<Ctxt> for Weaken<'tcx, crate::utils::OwnedPlace<'tcx>, FromCap, ToCap>
{
    type Repr = Weaken<'static, String, FromCap, ToCap>;
    fn debug_repr(&self, ctxt: Ctxt) -> Self::Repr {
        Weaken {
            place: self.place.place().display_string(ctxt),
            from: self.from,
            to: self.to,
            _marker: PhantomData,
        }
...
impl<
    ...
    FromCap: DisplayWithCtxt<Ctxt>,
    ToCap: DisplayWithCtxt<Ctxt>,
    Ctxt: Copy,
> DisplayWithCtxt<Ctxt> for Weaken<'tcx, P, FromCap, ToCap>
{
...
pub trait PcgDataTypes<'tcx>: std::fmt::Debug + Clone {
    ...
    type OwnedPlace: std::fmt::Debug + serde::Serialize = crate::utils::OwnedPlace<'tcx>;
...
impl PcgDataTypes<'_> for DebugDataTypes {
    type Local = String;
    type Place = String;
    type OwnedPlace = String;
...
pub enum RepackType {
    Real,
    Materialized(EdgeMutability),
}

impl<Ctxt> DebugRepr<Ctxt> for RepackType {
    type Repr = RepackType;
    fn debug_repr(&self, _ctxt: Ctxt) -> Self::Repr {
        *self
    }
}

impl<Ctxt: Copy> DisplayWithCtxt<Ctxt> for RepackType {
    fn display_output(&self, ctxt: Ctxt, mode: OutputMode) -> DisplayOutput {
        match self {
            RepackType::Real => DisplayOutput::Text("Real".into()),
            RepackType::Materialized(mutability) => mutability.display_output(ctxt, mode),
        }
    }
}

pub trait RepackDataTypes<'tcx>: PcgDataTypes<'tcx> {
    type ExpandCapability: std::fmt::Debug + serde::Serialize = RepackType;
}

impl RepackDataTypes<'_> for DebugDataTypes {}
