...
impl<N> CapabilityKind<N> {
    pub(crate) fn into_owned_capability(self) -> Option<OwnedCapability> {
        match self {
            CapabilityKind::ShallowExclusive => Some(OwnedCapability::Shallow),
            CapabilityKind::Write => Some(OwnedCapability::Uninitialized),
            CapabilityKind::Exclusive => Some(OwnedCapability::Deep),
            _ => None,
        }
    }
}
...
impl<N> From<OwnedCapability> for CapabilityKind<N> {
    fn from(cap: OwnedCapability) -> Self {
        match cap {
            ...
            OwnedCapability::Uninitialized => CapabilityKind::Write,
