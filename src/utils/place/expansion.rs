...
impl<'tcx, D: std::fmt::Debug> std::ops::Index<mir::PlaceElem<'tcx>> for PlaceExpansion<'tcx, D> {
    ...
    fn index(&self, index: mir::PlaceElem<'tcx>) -> &Self::Output {
        self.elems_data()
            ...
            .unwrap_or_else(|| panic!("Index {index:?} not found in PlaceExpansion {self:?}"))
            .1
            .unwrap()
    }
}
...
impl<'tcx> PlaceExpansion<'tcx> {
        ...
        'tcx: 'a,
    {
        Self::from_vec(
            places
                .into_iter()
                .map(|place| (place.last_projection().unwrap().1, ()))
                .collect(),
        )
    }
}
...
impl<'tcx, D> PlaceExpansion<'tcx, D> {
    ...
    pub(crate) fn elems_data<'slf>(&'slf self) -> Vec<(PlaceElem<'tcx>, Option<&'slf D>)> {
        self.map_elems_data(Some, |_| None)
    }
    ...
    pub(crate) fn elems_data_mut<'slf>(
        ...
        match self {
            ...
            PlaceExpansion::Guided(RepackGuide::ConstantIndex(c, data)) => {
                ...
                elems.extend(c.other_elems().iter().map(|e| (*e, None)));
                elems
            }
    ...
    pub(crate) fn try_map_data<'slf, R>(
        ...
        match self {
            PlaceExpansion::Fields(fields) => {
                ...
                for (field_idx, (ty, data)) in fields {
                    let new_data = f(data)?;
    ...
    pub(crate) fn map_elems_data<'slf, R>(
        ...
        match self {
            ...
            PlaceExpansion::Guided(RepackGuide::ConstantIndex(c, data)) => {
                ...
                elems.extend(c.other_elems().iter().map(|e| (*e, default(data))));
                elems
            }
            PlaceExpansion::Guided(guided) => {
    ...
    pub(crate) fn from_vec(expansions: Vec<(PlaceElem<'tcx>, D)>) -> Self {
        {
            let mut fields = BTreeMap::new();

            for (elem, data) in expansions {
                match elem {
                    PlaceElem::Field(field_idx, ty) => {
                        fields.insert(field_idx, (ty, data));
                    }
                    PlaceElem::Deref => return PlaceExpansion::Deref(data),
                    other => {
                        let repack_guide: RepackGuide = other.into();
                        return PlaceExpansion::Guided(
                            repack_guide.as_non_default().unwrap().map_data(|()| data),
                        );
                    }
                }
            }
