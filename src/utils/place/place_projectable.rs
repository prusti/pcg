use crate::{
    error::{IllegalProjection, PcgError},
...
impl<'a, 'tcx: 'a, Ctxt: HasCompilerCtxt<'a, 'tcx>> PlaceProjectable<'tcx, Ctxt> for Place<'tcx> {
    fn project_deeper(&self, elem: PlaceElem<'tcx>, ctxt: Ctxt) -> Result<Self, PcgError<'tcx>> {
        self.project_elem(elem, ctxt)
            .map_err(|err| PcgError::internal(format!("{err:?}")))
    }
    fn iter_projections(&self, ctxt: Ctxt) -> Vec<(Self, PlaceElem<'tcx>)> {
        self.0
            .iter_projections()
            .map(|(place, elem)| (Place::new(place.local, place.projection, ctxt), elem))
            .collect()
    }
}
...
{
    fn project_deeper(&self, elem: PlaceElem<'tcx>, ctxt: Ctxt) -> Result<Self, PcgError<'tcx>> {
        Ok(Self(self.0.project_deeper(elem, ctxt)?))
    }
    fn iter_projections(&self, ctxt: Ctxt) -> Vec<(Self, PlaceElem<'tcx>)> {
        self.0
...
impl<'tcx> Place<'tcx> {
        ...
        'tcx: 'a,
    {
        ...
        IllegalProjection::check(base_ty, elem)?;
        ...
        let deeper = self.0.project_deeper(&[corrected_elem], ctxt.tcx());
        Ok(Place::new_unchecked(deeper.local, deeper.projection))
    }
}
