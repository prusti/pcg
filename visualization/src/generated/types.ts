 ...
 * action).
 */
export type ActionKindWithDebugCtxt<T> = {
  kind: T;
...
export type BorrowPcgActionKindDebugRepr = {
  type: BorrowPcgActionKindDiscriminants;
  data: string;
};
 ...
 */
export type BorrowPcgActionKindDiscriminants =
...
export type DataflowStmtPhase =
  | "Initial"
  | { EvalStmt: EvalStmtPhase }
  | { Join: BasicBlock };

export type DotFileAtPhase<PhaseKey> = {
  phase: PhaseKey;
  filename: PathToDotFile;
};

export type EvalStmtData<T> = {
  pre_operands: T;
  post_operands: T;
  pre_main: T;
  post_main: T;
...
export type PcgActionDebugRepr =
  | {
      type: "Owned";
      data: ActionKindWithDebugCtxt<RepackOp<string, string, string>>;
    }
  | {
      type: "Borrow";
      data: ActionKindWithDebugCtxt<BorrowPcgActionKindDebugRepr>;
    };

export type PcgBlockVisualizationData = {
  statements: PcgStmtVisualizationData[];
  successors: { [key in BasicBlock]: PcgSuccessorVisualizationData };
};

export type PcgStmtVisualizationData = {
  actions: EvalStmtData<PcgActionDebugRepr[]>;
  graphs: StmtGraphs<StringOf<DataflowStmtPhase>>;
};

export type PcgSuccessorVisualizationData = { actions: PcgActionDebugRepr[] };

export type PcgVisualizationData = {
  [key in BasicBlock]: PcgBlockVisualizationData;
};

export type RegainedCapability<Place> = {
  place: Place;
  capability: CapabilityKind;
};

export type RepackCollapse<Place, Guide> = {
  to: Place;
  capability: CapabilityKind;
  guide: Guide | null;
};

export type RepackExpand<Place, Guide> = {
  from: Place;
  guide: Guide | null;
  capability: CapabilityKind;
};
...
export type StmtGraphs<PhaseKey> = {
  at_phase: DotFileAtPhase<PhaseKey>[];
  actions: EvalStmtData<PathToDotFile[]>;
};

export type ValidityConditionsDebugRepr = {
  branch_choices: BranchChoicesDebugRepr[];
};
 ...
 */
export type Weaken<Place, ToCap> = {
  place: Place;
  from: CapabilityKind;
  to: ToCap;
};
